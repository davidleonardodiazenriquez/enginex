...
from core.models import ContractDocument, ExtractedField, ExtractionRun
...
def process_run(run):
    try:
        ...
        try:
            resolve_document_location(run.document_id)
        except Exception as exc:
            # Valid extraction remains available even if the separate matching step fails.
            ContractDocument.objects.filter(pk=run.document_id).exclude(association_status__in=["manual", "outside"]).update(
                association_status="review", association_details={"reason": "Location matching could not complete. Review the premises and choose a location."})
            logger.warning("Location matching failed: document=%s type=%s", run.document_id, type(exc).__name__)
    except Exception as exc:
