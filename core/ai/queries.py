...
from core.portfolio.context import asset_context
from core.presentation import portfolio_text, stored_record_search
from core.record_fields import FIELDS
from core.reports.selectors import portfolio_summary
from core.feedback.selectors import feedback_summary
...
def overview():
    ...
    for asset in Asset.objects.select_related("metrics").prefetch_related("tenant_revenues", "vacancies"):
        ...
        assets.append({"asset": context["asset"], "field_sources": asset.field_sources,
                       ...
                       "resident_feedback": context["resident_feedback"],
    ...
    return {"source": "Live application database", "retrieved_at": timezone.now().isoformat(),
            ...
            "resident_feedback": feedback_summary(ResidentFeedback.objects.all()),
...
def selected_records(args):
    ...
    if search:
        matches = Q()
        for term in {search, stored_record_search(search)}:
            matches |= Q(code__icontains=term) | Q(data__tenant_name__icontains=term) | Q(data__unit_reference__icontains=term)
    ...
    def matches(row):
        for item in filters:
            ...
            if actual is None:
                return False
            if op == "contains":
                ok = str(expected).casefold() in str(actual).casefold() or str(expected).casefold() in str(portfolio_text(actual)).casefold()
                ...
                ok = compare(str(actual).casefold(), str(expected).casefold(), op) or compare(str(portfolio_text(actual)).casefold(), str(expected).casefold(), op)
            if not ok:
                return False
        return True
...
def aggregate_records(args):
    ...
    if chart_type != "none":
        result["chart"] = {"type": chart_type, "title": text(args.get("title", f"{metric.title()} by {group_by.replace('_', ' ')}"), 120),
                           "rows": [{**row, "label": portfolio_text(row["label"])} for row in shown], "unit": unit,
                           "provenance": result["provenance"], "display_source": "Portfolio records; document evidence and summary metrics are separate.", "matched_records": len(rows),
...
def contract_evidence(args):
    ...
    for field in query[offset:offset + limit]:
        ...
        rows.append({"document": doc.title, "document_id": str(doc.pk), "field": field.name, "value": field.value,
                     ...
                     "association_status": doc.get_association_status_display(),
...
def document_library(args):
    ...
    for doc in docs.order_by("title")[offset:offset + limit]:
        ...
        rows.append({"document": doc.title, "document_id": str(doc.pk), "pages": doc.page_count, "record": doc.record.code,
                     ...
                     "association_status": doc.get_association_status_display(), "association_reason": doc.association_details.get("reason", ""),
                     ...
                     "associations": list(doc.associations.values("record_code", "location_name", "reason", "created_at", "method", "evidence")[:5]),
